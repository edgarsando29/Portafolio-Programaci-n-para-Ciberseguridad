function Ver-StatusPerfil{  
    ...
    Write-Host "- - - - - - Info del Status del Perfil - - - - - -"
...
param([Parameter(Mandatory)] [ValidateSet("Public","Private")] [string] $perfil)
Write-Host "- - - - - - Cambio Status del Perfil - - - - - -"
$status = Get-NetFirewallProfile -Name $perfil  
Write-Host "Perfil:" $perfil  
if($status.enabled){  
...
$puerto = Read-Host -Prompt "Cuál puerto quieres bloquear? (Ingresa * para salir de función)"
if($puerto -ne "*"){
...
$opc = Read-Host -Prompt "Deseas eliminar esta regla [Y] Si [N] No (Ingresa * para salir de función)"
if($opc -eq "*"){
break
}
else
